...
6. `Saha Sorumlusu` bölümünde lokasyonun Head of Master bilgisinin otomatik `Ali Kaya` veya `hom1@workplan.local` olarak geldiğini doğrula.
...
3. `Ekip` bölümünde `Ekip tipi` alanından yalnızca `Düz işçi` seçeneğini işaretle.
4. `İşi Başlat` butonuna bas.
5. Onay modalini onayla.

Beklenen sonuç: Başarılı mesaj görünür, durum `Devam Ediyor` olur, `Gün sonu ilerleme gir` formu görünür. Ekip bölümünde seçilen tek ekip tipi listelenir.
...
1. `to1@workplan.local` veya `admin@workplan.local` ile giriş yap.
2. `Ekip Tipleri` sayfasına git.
3. `Manuel E2E Ekip Tipi` adında yeni kayıt oluştur.
4. Kaydın listede aktif olarak göründüğünü doğrula.
5. Kaydı `Manuel E2E Ekip Tipi Güncel` olarak düzenle.
6. Kaydı pasifleştir, sonra tekrar aktifleştir.

Beklenen sonuç: Kayıt oluşturma, düzenleme ve aktif/pasif değiştirme işlemleri başarılı olur. Sayfada proje, bölge, lokasyon, personel ref veya üye ekleme alanı görünmez.
...
2. `hom1@workplan.local` ile plana gir, ekip tipi seçip `İşi Başlat`.
...
Amaç: PM reddedince Site Chief onaylı işin Site Chief kuyruğuna geri dönmesini kontrol etmek.

1. E2E-02 ile yeni bir plan oluştur.
2. HoM ile ekip tipi seçerek başlat ve geçerli gerçekleşme gir.
...
Beklenen sonuç: İş durumu tekrar `Site Chief Onayında` olur, red nedeni durum geçmişine yazılır ve `sc1@workplan.local` ile giriş yapıldığında iş onay kuyruğunda kırmızı red uyarısıyla yeniden kontrol edilebilir.
...
6. Ekip tipi seçimi, `İşi Başlat`, gün sonu formu ve `Kaydet ve Onaya Sun` butonlarının taşmadan kullanılabildiğini doğrula.
